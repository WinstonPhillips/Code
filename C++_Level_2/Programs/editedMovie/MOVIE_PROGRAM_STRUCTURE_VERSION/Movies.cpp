...
{
	//dynamically create a new Movies array
	moviesArray = new Movie*[max];	// Pointer to an array of pointers
	maxMovies = max;
	numMovies = 0;
...
void Movies::displayMovieTitles()
{
	...
	{
		cout << "\nMOVIE " << (x+1) <<": ";
		movieTitle = moviesArray[x]->getmovieTitle();		// get mopvie title because it returns the title
		movieTitle->displayText();
	}
}
