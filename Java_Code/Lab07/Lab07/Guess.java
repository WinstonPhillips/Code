...
public class Guess
{
   ...
   public int[] reportResult(Guess secret)
   {
      ...
      List<Integer> secret_guess_ids = secret.getGuessIDs(); // array list for Integers
      List<Integer> current_guess_ids = getGuessIDs();      // array list of Integers
	...
	//make sure to account for the shifting of elements after a removal (decrement your counter)
      ...
      {
         if (Integer.valueOf(secret_guess_ids.get(i)) == Integer.valueOf(current_guess_ids.get(i)))
         {
            black_count++;
            secret_guess_ids.remove(i);
            current_guess_ids.remove(i);
            i--; // and then will be incremented, will be in same position
         }
      }
	...
	//once elements have been removed, break out of the inner loop
      ...
      for (int i = 0; i < secret_guess_ids.size(); i++)
      {
         ...
         {
            if (Integer.valueOf(secret_guess_ids.get(i)) == Integer.valueOf(current_guess_ids.get(j)))
            {
               white_count++;
