...
{
   private ArrayList<Student> students; // a list of students, each having their own lab list
   private int current_labID = 1;
   private int max_rank = 1;  //you don't have to worry about this instance variable
   private int current_lab = 0;
   ...
   private int determineCurrentLabNumber()
   {
      ...
      //for efficiency, store this value in an instance variable to avoid computing it over and over
         ...
         {
            if(student.getNumLabs() > labNumber)
            {
               labNumber = student.getNumLabs();
            }
         }

            current_lab = labNumber; // based on instructions
   ...
   public String partnerList(int labID)
   {
      ...
      for (Student student : students)
      {
         ...
         try
         {
            present = student.isPresent(current_labID); //if the student is inactive, this can cause an exception
         }
         ...
         if (present)
         {
            ...
            //DO THIS
            int partnerID = student.getPartnerID(current_lab);
            String partnerName = "";
            if (partnerID > 0) // if partner exists
            {
   ...
   {
      students.get(studentID-1).addLab();
   }

   public void setGrade(int lab, int studentID, char grade)
   {
      Student student = students.get(studentID-1);

      student.setGrade(lab,grade);
   }
   ...
   private int determineMaxRank()  // gotta figure a way for the private access 
   {
      int maxRank = 1;
      for (Student student : students)
      {
