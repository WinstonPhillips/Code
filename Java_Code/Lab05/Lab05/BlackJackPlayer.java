...
{
   private ArrayList<BlackJackHand> hands; // an array list of several hands
   ...
   public void draw(Graphics g, int width, int height)
   {
      ...
      for(BlackJackHand hand: hands) // for each BlackJack hand in hands, do these steps while naming each hand hand
      {
   ...
   {
      boolean can_split = canSplit(); //All of this needs work

      if (can_split)
      {
         
          BlackJackHand[] newHands = hands.get(index-1).split(card1, card2); // ORDER MATTERS

          hands.remove(index-1);    // removes hand from hands arrayList // maybe index - 1

          hands.add(index-1, newHands[0]);   // add first split hand to first index

          hands.add(index-1, newHands[1]);   // add second split hand to second index // automatically accomodates for index-1
    ...
    *  Obtain the current hand using the index instance variable.
    *  Hit the hand.
   ...
   public void doubleDown(Card dealt)
   {
      ...
      hand.doubleDown(); // sets hand as doubled down
   }
