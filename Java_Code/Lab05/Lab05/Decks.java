...
public class Decks
{
   ...
   public Decks(int num)
   {
      ...
      for (int i = 0; i < numDecks; i++)
      {
         ...
         //you will need a counter to insert the Cards into the decks array
          for (int j = 0; j < STANDARD_DECK_SIZE; j++) // size = total number of cards
          {
            newCard = new Card(j+1);
            decks[count] = newCard; // MAYBE
            count++;
          }
      ...
      shuffle();
    //   Permutation myPerm = new Permutation(SIZE,SIZE); 

    //   Card[] newDeck = new Card[SIZE];

    //   for (int i = 0; i < SIZE; i++)
    //   {
    //      newDeck[myPerm.next() -1] = decks[i];
    //   }
   ...
   public void shuffle()
   {
      ...
      {
         newDeck[myPerm.next() -1] = decks[i - 1];
      }
