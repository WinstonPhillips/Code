...
public class BlackJackStrategy
{
   ...
   public char getMove(BlackJackHand player, BlackJackHand dealer) 
   {
      ...
      char optimal = 'S'; // char to return

      int row = 0;
      int col = 0;
      int player_value = player.handValue(); // value of the player's hand
      ...
      double numToConvert = strategy.getElement(row, col);
   ...
   public boolean shouldSplit(BlackJackHand player, BlackJackHand dealer)
   {
      ...
      if (player.handValue() == 2) // player.canSplit() &&
      {
        row = 38;
      }
      else // else if (player.canSplit())
      {
        row = ((player.handValue()) / 2) + 27;
      }
      

      double numToConvert = strategy.getElement(row +1 , col); // row is one based
